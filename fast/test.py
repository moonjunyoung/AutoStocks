...
def save_user_info_to_json(
    id_token, cano=None, app_key=None, app_secret=None, user_agent=None
):
    ...
    for stored_id_token_key, encoded_user_info in data.items():
        ...
        if stored_decrypted_id_token == id_token:
            ...
            if need_update:
                encoded_user_info = {
                    key: (
                        base64.b64encode(encrypt_data(value)).decode("utf-8")
                        if value
                        else None
                    )
    ...
    if not found:
        ...
        encoded_user_info = {
            key: (
                base64.b64encode(encrypt_data(value)).decode("utf-8") if value else None
            )
            for key, value in user_info.items()
        }
    ...
    try:
        with open("user_info.json", "r") as file:
            ...
            if id_token in data:
                ...
                user_info = {
                    key: decrypt_data(base64.b64decode(value))
                    for key, value in encoded_user_info.items()
                }
...
async def kakao_auth(
    request: Request, code: str = Query(...), response_class=HTMLResponse
):
    ...
    if "access_token" in token_response:
        ...
        save_user_info_to_json(
            id_token, cano=None, app_key=None, app_secret=None, user_agent=user_agent
        )
        ...
        return JSONResponse(
            {"status": "error", "message": "액세스 토큰을 발급받을 수 없습니다."}
        )
...
async def get_ui_card(request: Request):
    ...
    buy_orders = [{"code": "000080", "price": "20,500"}]
    return templates.TemplateResponse(
        "ui-card.html", {"request": request, "buy_orders": buy_orders}
    )
...
async def post_ui_card(
    request: Request, company_name: str = Form(...)
):  # 종목명 입력 받음
    # 종목명을 이용하여 종목코드 가져오기
    print(company_name)
    ...
    return templates.TemplateResponse(
        "ui-card.html", {"request": request, "sell_orders": sell_orders}
    )
...
async def register(
    ...
    print(
        f"계좌번호: {cano}, App Key: {app_key}, App Secret: {app_secret}, ID Token: {id_token}"
    )
