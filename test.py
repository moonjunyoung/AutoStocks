...
from service.common.token import Token
...
# HTML 파일 경로
html_file = "./presentation/templates/index.html"
...
@app.get("/naver-login")
async def naver_login(oauth_data: OauthData):
    platform = "naver"
    user = user_service.oauth_login(platform, oauth_data.token)
    jwt_token = Token.create_token_by_user(user)
    return jwt_token


@app.get("/kakao-login")
async def kakao_login(oauth_data: OauthData):
    platform = "kakao"
    user = user_service.oauth_login(platform, oauth_data.token)
    jwt_token = Token.create_token_by_user(user)
    return jwt_token
